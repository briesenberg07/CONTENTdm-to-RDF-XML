...
```
...`valueConstraint` is required, and it in turn requires `valueTemplateRefs` and must not have defaults. (Or defaults are simply *not required*?)
...
```
...its `valueConstraint` may not include `useValuesFrom` or `valueTemplateRefs`. (Or these are simply *not required*??)
