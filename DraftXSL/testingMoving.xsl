...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="metadata" mode="agent">
        <xsl:apply-templates select="record" mode="agent"/>
    </xsl:template>
    ...
    <!-- sub-template for agent file -->
    <xsl:template match="record" mode="agent">
        <xsl:apply-templates select="Photographer" mode="agent"/>
        <xsl:apply-templates select="Repository" mode="agent"/>
        <xsl:apply-templates select="StudioName" mode="agent"/>
    </xsl:template>
    ...
    <xsl:template match="Photographer" mode="agent">
        <xsl:for-each-group select="current()[text() != '' and text() != 'Unidentified' and text() != 'unidentified']"
            group-by="current()">
            <rdf:Description
    ...
    <xsl:template match="Repository" mode="agent">
        <xsl:for-each-group select="current()[text() != '']" group-by="current()">
            <rdf:Description
                rdf:about="{concat('http://doi.org/10.6069/uwlib.55.A.3.6#',translate(current-grouping-key(),' '',''.',''))}">
                <rdf:type rdf:resource="http://www.europeana.eu/schemas/edm/Agent"/>
            </rdf:Description>
    ...
    <xsl:template match="StudioName" mode="agent">
        <xsl:for-each-group select="current()[text() != '']" group-by="current()">
            <rdf:Description
                rdf:about="{concat('http://doi.org/10.6069/uwlib.55.A.3.6#',translate(current-grouping-key(),' '',''.',''))}">
                <rdf:type rdf:resource="http://www.europeana.eu/schemas/edm/Agent"/>
